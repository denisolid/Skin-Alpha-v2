...
export interface ResolveBitSkinsListingInput {
  ...
  readonly targetCanonicalItemId?: string;
  readonly targetItemVariantId?: string;
}

export interface BitSkinsCatalogLinkerRunContext {
...
@Injectable()
export class BitSkinsCatalogLinkerService {
  ...
  createRunContext(): BitSkinsCatalogLinkerRunContext {
    return {
  ...
  async resolveOrCreateMany(
    inputs: readonly ResolveBitSkinsListingInput[],
    runContext?: BitSkinsCatalogLinkerRunContext,
  ): Promise<readonly CatalogResolutionDto[]> {
    if (inputs.length === 0) {
      return [];
    }

    const uniqueInputs = new Map<string, ResolveBitSkinsListingInput>();
    ...
    const unresolvedInputs: ResolveBitSkinsListingInput[] = [];
    ...
    if (unresolvedInputs.length > 0) {
      ...
      unresolvedInputs.forEach((input, index) => {
        const catalogResolution = persistedResolutions[index];

        if (!catalogResolution) {
          return;
        }

        const resolvedMapping = this.applyTargetHint(input, catalogResolution);
        const cacheKey = this.buildCacheKey(input);
    ...
    return inputs.map((input) => {
      ...
      if (!resolvedMapping) {
        throw new Error(
          `BitSkins catalog linker failed to resolve "${input.marketHashName}".`,
        );
      }
  ...
  private applyTargetHint(
    input: ResolveBitSkinsListingInput,
    resolution: CatalogResolutionDto,
  ): CatalogResolutionDto {
    if (
      resolution.status === 'resolved' ||
      !input.targetCanonicalItemId ||
      !input.targetItemVariantId
    ) {
      return resolution;
    }
    ...
    return {
      ...resolution,
      status: 'resolved',
      confidence: Math.max(resolution.confidence, 0.99),
      reason: 'bitskins_exact_overlap_target',
      warnings: [...resolution.warnings, 'Resolved via BitSkins overlap target hint.'],
      canonicalItemId: input.targetCanonicalItemId,
      itemVariantId: input.targetItemVariantId,
    };
  }
  ...
  private writeToCache(cacheKey: string, value: CatalogResolutionDto): void {
    if (
      ...
      BitSkinsCatalogLinkerService.MAX_CACHE_ENTRIES
    ) {
    ...
    this.resolutionCache.set(cacheKey, {
      value,
      expiresAtMs: Date.now() + BitSkinsCatalogLinkerService.CACHE_TTL_MS,
    });
  }

  private buildCacheKey(input: ResolveBitSkinsListingInput): string {

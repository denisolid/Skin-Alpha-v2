...
@Injectable()
export class WaxpeerRateLimitService {
  constructor(
    @Inject(REDIS_CLIENT)
  ...
  async recordResponse(headers: Headers): Promise<HeaderRateLimitSnapshot> {
    ...
    const now = Date.now();
    const nextState: StoredRateLimitState = {
      limit: snapshot.limit ?? this.configService.waxpeerRateLimitMaxRequests,
      remaining:
        snapshot.remaining ?? this.configService.waxpeerRateLimitMaxRequests,
      resetsAt:
        snapshot.resetAt?.getTime() ??
        now + this.configService.waxpeerRateLimitWindowSeconds * 1000,
  ...
  async markRateLimited(retryAfterSeconds?: number): Promise<void> {
    const now = Date.now();
    const waitSeconds =
      retryAfterSeconds ?? this.configService.waxpeerRateLimitWindowSeconds;

    await this.writeState(
      {
        limit: this.configService.waxpeerRateLimitMaxRequests,
        remaining: 0,
  ...
  private async readState(now: number): Promise<StoredRateLimitState> {
    ...
    if (!rawValue) {
      return {
        limit: this.configService.waxpeerRateLimitMaxRequests,
        remaining: this.configService.waxpeerRateLimitMaxRequests,
        resetsAt:
          now + this.configService.waxpeerRateLimitWindowSeconds * 1000,
      };
    }
  ...
  private getRedisKey(): string {
    return 'source:waxpeer:rate-limit:mass-info';
  }

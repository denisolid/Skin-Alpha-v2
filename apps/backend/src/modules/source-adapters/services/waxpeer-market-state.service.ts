...
@Injectable()
export class WaxpeerMarketStateService {
  constructor(
  ...
  async reconcileAndRebuild(
    ...
    const source = await this.sourceRecordService.resolveByKey('waxpeer');
    ...
    const marketStates: NormalizedMarketStateDto[] = affectedVariants.map(
      (variant) => {
        ...
        return {
          ...
          currency: variant.currencyCode || this.configService.waxpeerCurrency,
  ...
  private deriveConfidence(volume: number, anchorCount: number): number {
    const baseConfidence = Math.min(0.9, 0.44 + Math.log10(volume + 1) / 4.5);
  ...
  private extractAnchorPriceMinor(attributes: unknown): number | undefined {
    ...
    const candidate = record.steamPriceMinor;

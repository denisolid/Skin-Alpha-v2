...
@Injectable()
@Processor(SKINPORT_SYNC_SALES_HISTORY_QUEUE_NAME)
export class SkinportSyncSalesHistoryProcessor extends WorkerHost {
  constructor(
  ...
  async process(job: Job<SkinportSyncJobData, void, string>): Promise<void> {
    if (job.name !== SKINPORT_SYNC_SALES_HISTORY_JOB_NAME) {
      return;
    }

    await this.skinportSyncService.syncSalesHistory(job.data);
  }
}
